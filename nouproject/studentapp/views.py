...
from adminapp.models import Materials
        ...
        if request.session['rollno']!=None:
            ...
            if request.method=='POST':
                ...
                if newpassword!=confirmpassword:
                    messages.error(request,'New password and confirmpassword are not matched')
...
def viewsmat(request):
    try:
        if request.session['rollno']!=None:
            ...
            branch=stu.branch
            year=stu.year
            mat=Materials.objects.filter(program=program,branch=branch,year=year,materialtype='smat')
